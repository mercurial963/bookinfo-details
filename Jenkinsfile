      ...
      steps {
        container('helm'){
          script {
            ...
            sh "helm template -f helm-values/values-bookinfo-${ENV_NAME}-details.yaml \
            --set extraEnv.COMMIT_ID=${scmVars.GIT_COMMIT} \
            --namespace ${ENV_NAME} bookinfo-${ENV_NAME}-details helm/ \
        ...
        container('docker'){
          script {
            docker.withRegistry('https://ghcr.io', 'registry-bookinfo'){ //registry-bookinfo is user with token
                          // build and push
              docker.build('ghcr.io/mercurial963/bookinfo-details:${ENV_NAME}').push()
      ...
      steps {
        container('jnlp'){
          script {
            ...
            // def image = 'ghcr.io/mercurial963/bookinfo-details:${ENV_NAME}'
                         
            writeFile file: 'anchore_images', text: "ghcr.io/mercurial963/bookinfo-details:${ENV_NAME}"
        ...
        container('helm'){
          script {
            // withKubeConfig([credentialsId: 'config']){ //add kubeconfig to secret file
              sh "helm upgrade --install -f helm-values/values-bookinfo-${ENV_NAME}-details.yaml --wait \
              --set extraEnv.COMMIT_ID=${scmVars.GIT_COMMIT} \
              --namespace ${ENV_NAME} bookinfo-${ENV_NAME}-details helm/"
              // }// withCredentials
